...
export const PilesContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const currentPileName = decodeURIComponent(
      location.pathname.split(/[/\\]/).pop()
    );

    changeCurrentPile(currentPileName);
    // Obsidian port: piles.json loads async and we mount directly at
    // /pile/<name>, so re-resolve once `piles` is populated (otherwise
    // currentPile stays null and everything reading currentPile.path throws).
